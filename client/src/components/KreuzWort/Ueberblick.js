...
const Details = ({ 
  ...
  untersatzTypen.subtypliste(satz, subtyp, "Ueberblick").map((subtyp, ix) => (  // alle möglichen Untersatztypen durchlaufen
    <tr key={ix} style={{ color: `${satztypen[subtyp]}` }}>
      <td>
      { // Überschrift des Basissatzes
          stopworte.includes(wort.wort)? null:
          !wort.satze ? "kein Satz!" : ( 
          wort.satze.filter(sa => sa.typ.wort===subtyp).map((sa, ix) => (
            <a
              key={sa._id + ix}
      ...
      {
        // (() => {
        //   console.log("{wort, worte}", {wort, worte});
        // })()
      }
      </td>
      ...
      { // Worte im Basissatz
        !worte ? "keine Worte!" : ( 
          worte.slice(0, maxWorte).map((wo, ix) => ( 
            stopworte.includes(wo.wort)? <td key={!wo? satz._id+ix: satz._id + wo._id + ix}></td>:
            <td key={!wo? satz._id+ix: satz._id + wo._id + ix}>
            { // Satze mit dem gewünschten Subtyp, für die die Worte im Basissatz die Überschrift (hier: sa.wort.wort===wo.wort) bilden
              !wo? null:
              wo.satze.filter(sa => sa && sa.typ.wort===subtyp).map((sa, ix) => (
                <a
                  key={!wo? ix: wo._id + ix}
                  onClick={() => handleClick.left(sa.wort, sa.typ, zelleid)}
                  onContextMenu={(e) => { e.preventDefault(); handleClick.right(sa.wort, sa.typ); }}
